...
// v.2.6 20091215 WCL addition
// Copyright (c) WebSoft, 2002-2009. All rights reserved.

var g_sComponent = "Компонент ";
var g_sIsNotInstalled = " не установлен.";
var g_sLoadingImages = "Загрузка картинок";
...
function ShowObject(sTargetId, nTransition, nDuration)
{
	...
	try
	{
		...
		if (oObject)
		{
			...
			HandleEvt("EVENT_OBJECT_ONLOAD", null); // Fire object's internal event handler, if any
		...
		if (sSoundSrc.indexOf('.swf') != -1)
		{
			...
			if (oInternalPlayer != null)
			{
				try
				{
					...
					{
						oInternalPlayer.Stop();//Opera workaround
					}
				}
				catch(e)
				{}
...
// Run
function Run(bPreloadImages, sModuleId)
{
	...
	else
	{
		...
		{
			var aParts = window.location.search.split("&");
			for(var i=0; i<aParts.length; i++)
			{
				var aChunks = aParts[i].split("=");
				switch(aChunks[0])
				{
					case "slideid":
					{
	...
	if (sSlideId == null)
	{
		var bShowSplash = LoadSplash((sModuleId==null) ? null : sModuleId);
		if (bShowSplash == false)
			StartModule(sSlideId, sFrameId, ((sModuleId==null) ? null : sModuleId));
	}
	else
		StartModule(sSlideId, sFrameId, ((sModuleId==null) ? null : sModuleId));
}
...
// LoadSplash
function LoadSplash(sModuleId)
{
	var oDoc = CreateDOMDocument();

	var bLoaded = false;
	var sURL = "splash.xml";
	if(sModuleId!=null) sURL += "?module_id=" + sModuleId;
	if (g_isMSIE)
	{
		oDoc.async = false;
...
// StartModule
function StartModule(sSlideId, sFrameId, sModuleId)
{
	g_oDoc = CreateDOMDocument();

	var bLoaded = false;
	var sURL = "runtime.xml";
	if (sModuleId!=null)
		sURL += "?module_id=" + sModuleId;
	else if (g_sModuleId!=null)
		sURL += "?module_id=" + g_sModuleId;
	if (g_isMSIE)
	{
		g_oDoc.async = false;
		bLoaded = g_oDoc.load(sURL);
	}
	else if (g_isFirefox)
	{
		var sWindowLocation = window.location;
