...
<!--							$val = normalize-space(columns/column[number($sort.column)]/column_value/text())
								$firsthalf = substring-before($val,' ')
								$year = substring-after(substring-after($firsthalf,'.'),'.')
								$month = substring-before(substring-after($firsthalf,'.'),'.')
								$day = substring-before($firsthalf,'.')
								$secondhalf = substring-after($val,' ')
								$hour = substring-before($secondhalf,':')
								$min = substring-before(substring-after($secondhalf,':'),':')
								$sec = substring-after(substring-after($secondhalf,':'),':')
								<xsl:sort select="number(concat($year,$month,$day,$hour,$min,$sec))"/>
-->
	...
	<xsl:template match="report_result">
		...
		<table cellpadding="0" cellspacing="0" border="0">
			...
			<xsl:choose>
				<xsl:when test="$filter.enable='1' and $agg.column='0'">

					<tr id="wsgr_filters_row">
						<xsl:if test="$settings.display_row_numbers='1'">
							<td class="wsgr_filter_icon">&gt;</td>
						</xsl:if>
						<xsl:for-each select="header/columns/column">
							<xsl:variable name="current.column" select="position()"/>
							<xsl:variable name="current.sort.type" select="sort/text()"/>
							<xsl:variable name="current.key">key.<xsl:value-of select="$current.column"/></xsl:variable>
							<xsl:variable name="current.separator" select="concat(string($current.column),'~')"/>
							<xsl:variable name="selected.sub1" select="substring-after($filter.values, $current.separator)"/>
							<xsl:variable name="selected.value" select="substring-before($selected.sub1, '+')"/>
							<td>
								<xsl:attribute name="id">WSGR_select_td_<xsl:value-of select="$current.column"/></xsl:attribute>
								<xsl:attribute name="class">wsgr_filters<xsl:if test="$current.column=1">_first</xsl:if></xsl:attribute>
								<select onchange="WSGR_FilterApply(this); return false;">
									<xsl:attribute name="class">wsgr_filter_select<xsl:if test="$selected.value!='0' and $selected.value!=''">_active</xsl:if></xsl:attribute>
									<xsl:attribute name="column"><xsl:value-of select="$current.column"/></xsl:attribute>
									<option selected="1" value="0">*</option>

									<xsl:choose>
										<xsl:when test="$current.sort.type='date'">

											<xsl:for-each select="../../../rows/row[count(. | key( $current.key, columns/column[$current.column])[1]) = 1]">
												<xsl:sort data-type="number" order="ascending" select="number(concat(substring-after(substring-after(normalize-space(columns/column[number($current.column)]/column_value/text()),'.'),'.') , substring-before(substring-after(normalize-space(columns/column[number($current.column)]/column_value/text()),'.'),'.') , substring-before(normalize-space(columns/column[number($current.column)]/column_value/text()),'.')))"/>
												<option>
													<xsl:attribute name="value"><xsl:value-of select="position()"/></xsl:attribute>
													<xsl:if test="string(position())=$selected.value"><xsl:attribute name="selected">1</xsl:attribute></xsl:if>
													<xsl:value-of select="columns/column[$current.column]/column_value/text()"/>
												</option>
											</xsl:for-each>

										</xsl:when>
										<xsl:when test="$current.sort.type='time'">

											<xsl:for-each select="../../../rows/row[count(. | key( $current.key, columns/column[$current.column])[1]) = 1]">
												<xsl:sort data-type="number" order="ascending" select="number(concat(substring-before(normalize-space(columns/column[number($current.column)]/column_value/text()),':'),substring-before(substring-after(normalize-space(columns/column[number($current.column)]/column_value/text()),':'),':'),substring-after(substring-after(normalize-space(columns/column[number($current.column)]/column_value/text()),':'),':')))"/>
												<option>
													<xsl:attribute name="value"><xsl:value-of select="position()"/></xsl:attribute>
													<xsl:if test="string(position())=$selected.value"><xsl:attribute name="selected">1</xsl:attribute></xsl:if>
													<xsl:value-of select="columns/column[$current.column]/column_value/text()"/>
										...
										<xsl:when test="$current.sort.type='datetime'">

											<xsl:for-each select="../../../rows/row[count(. | key( $current.key, columns/column[$current.column])[1]) = 1]">
												<xsl:sort data-type="number" order="ascending" select="number(concat(substring-after(substring-after(substring-before(normalize-space(columns/column[number($current.column)]/column_value/text()),' '),'.'),'.'),substring-before(substring-after(substring-before(normalize-space(columns/column[number($current.column)]/column_value/text()),' '),'.'),'.'),substring-before(substring-before(normalize-space(columns/column[number($current.column)]/column_value/text()),' '),'.'),substring-before(substring-after(normalize-space(columns/column[number($current.column)]/column_value/text()),' '),':'),substring-before(substring-after(substring-after(normalize-space(columns/column[number($current.column)]/column_value/text()),' '),':'),':'),substring-after(substring-after(substring-after(normalize-space(columns/column[number($current.column)]/column_value/text()),' '),':'),':')))"/>
												<option>
													<xsl:attribute name="value"><xsl:value-of select="position()"/></xsl:attribute>
													<xsl:if test="string(position())=$selected.value"><xsl:attribute name="selected">1</xsl:attribute></xsl:if>
													<xsl:value-of select="columns/column[$current.column]/column_value/text()"/>
										...
										<xsl:otherwise>
											...
											<xsl:for-each select="../../../rows/row[count(. | key( $current.key, columns/column[$current.column])[1]) = 1]">
												<xsl:sort select="columns/column[number($current.column)]/column_value/text()" data-type="{$current.sort.type}" order="ascending"/>
												<option>
													<xsl:attribute name="value"><xsl:value-of select="position()"/></xsl:attribute>
													<xsl:if test="string(position())=$selected.value"><xsl:attribute name="selected">1</xsl:attribute></xsl:if>
													<xsl:value-of select="columns/column[$current.column]/column_value/text()"/>
					...
					<xsl:choose>
						<xsl:when test="$sort.type='date'">
							...
							<xsl:for-each select="rows/row">
								<xsl:sort select="number(concat(substring-after(substring-after(normalize-space(columns/column[number($sort.column)]/column_value/text()),'.'),'.') , substring-before(substring-after(normalize-space(columns/column[number($sort.column)]/column_value/text()),'.'),'.') , substring-before(normalize-space(columns/column[number($sort.column)]/column_value/text()),'.')))" data-type="number" order="{$sort.order}"/>
								<xsl:if test="(position() &gt;= number($page.start)) and (position() &lt;= (number($page.start)+number($page.size)-1))">

									<xsl:variable name="f.column.separator">+</xsl:variable>
									<xsl:variable name="f.text.separator">~~~</xsl:variable>
									<xsl:variable name="f.check">
										<xsl:for-each select="columns/column">
											<xsl:variable name="f.column" select="position()"/>
											<xsl:variable name="f.separator" select="concat($f.column.separator, string($f.column), $f.text.separator)"/>
											<xsl:variable name="f.sub1" select="substring-after($filter.strings, $f.separator)"/>
											<xsl:variable name="f.text" select="substring-before($f.sub1, $f.column.separator)"/>
											<xsl:if test="$f.text!=''">
												<xsl:if test="$f.text!=column_value/text()"><xsl:value-of select="$f.column"/></xsl:if>
											</xsl:if>
										</xsl:for-each>
									</xsl:variable>
									<xsl:if test="$f.check=''">
						...
						<xsl:when test="$sort.type='time'">

							<xsl:for-each select="rows/row">
								<xsl:sort data-type="number" order="{$sort.order}" select="number(concat(substring-before(normalize-space(columns/column[number($sort.column)]/column_value/text()),':'),substring-before(substring-after(normalize-space(columns/column[number($sort.column)]/column_value/text()),':'),':'),substring-after(substring-after(normalize-space(columns/column[number($sort.column)]/column_value/text()),':'),':')))"/>
								<xsl:if test="(position() &gt;= number($page.start)) and (position() &lt;= (number($page.start)+number($page.size)-1))">
						...
						<xsl:when test="$sort.type='datetime'">

							<xsl:for-each select="rows/row">
								<xsl:sort data-type="number" order="{$sort.order}" select="number(concat(substring-after(substring-after(substring-before(normalize-space(columns/column[number($sort.column)]/column_value/text()),' '),'.'),'.'),substring-before(substring-after(substring-before(normalize-space(columns/column[number($sort.column)]/column_value/text()),' '),'.'),'.'),substring-before(substring-before(normalize-space(columns/column[number($sort.column)]/column_value/text()),' '),'.'),substring-before(substring-after(normalize-space(columns/column[number($sort.column)]/column_value/text()),' '),':'),substring-before(substring-after(substring-after(normalize-space(columns/column[number($sort.column)]/column_value/text()),' '),':'),':'),substring-after(substring-after(substring-after(normalize-space(columns/column[number($sort.column)]/column_value/text()),' '),':'),':')))"/>
								<xsl:if test="(position() &gt;= number($page.start)) and (position() &lt;= (number($page.start)+number($page.size)-1))">
		...
		<span id="WSGR_TotalCount" style="display: none"><xsl:value-of select="count(rows/row)"/></span>
	</xsl:template>
			...
			<table cellpadding="0" cellspacing="0" border="0">
				...
				<tr>
					<td>
						...
						<xsl:attribute name="class">
							<xsl:choose>
								<xsl:when test="$column.number=$sort.column">
									<xsl:if test="$sort.order='ascending'">wsgr_sort_asc</xsl:if>
									<xsl:if test="$sort.order='descending'">wsgr_sort_desc</xsl:if>
								</xsl:when>
								<xsl:otherwise>wsgr_sort_none</xsl:otherwise>
							</xsl:choose>
						</xsl:attribute>
					...
					<td class="wsgr_header_text">
						...
						<xsl:attribute name="onclick">WSGR_ReSort(this); return false;</xsl:attribute>
