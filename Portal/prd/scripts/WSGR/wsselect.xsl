...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="report_result">
		...
		<select onchange="WSGR_FilterApply(this); return false;">
			<xsl:attribute name="column"><xsl:value-of select="$agg.column"/></xsl:attribute>
			<option selected="1" value="0">*</option>
			<xsl:choose>
				<xsl:when test="$current.sort.type='date'">

					<!--<xsl:for-each select="rows/row/columns/column[number($agg.column)]/column_value[not(text()=../preceding-sibling::rows/row/columns/column[number($agg.column)]/column_value)]">
						<xsl:sort select="number(concat(substring-after(substring-after(rows/row/columns/column[number($agg.column)]/column_value/text(),'.'),'.') , substring-before(substring-after(rows/row/columns/column[number($agg.column)]/column_value/text(),'.'),'.') , substring-before(rows/row/columns/column[number($agg.column)]/column_value/text(),'.')))" data-type="number" order="ascending"/>
					...
					<xsl:for-each select="rows/row[count(. | key( $current.key, columns/column[number($agg.column)])[1]) = 1]">
						...
						<option><xsl:value-of select="columns/column[number($agg.column)]/column_value/text()"/></option>
					</xsl:for-each>

				</xsl:when>
				<xsl:otherwise>

					<!--<xsl:for-each select="rows/row/columns/column[number($agg.column)]/column_value[not(text()=../../../preceding-sibling::rows/row/columns/column[number($agg.column)]/column_value/text())]">
						<xsl:sort select="text()" data-type="{$current.sort.type}" order="ascending"/>
						<option><xsl:value-of select="./text()"/></option>
					</xsl:for-each>-->


				<xsl:for-each select="rows/row[count(. | key( $current.key, columns/column[number($agg.column)])[1]) = 1]">
					<xsl:sort select="columns/column[number($agg.column)]/column_value" data-type="{$current.sort.type}" order="ascending"/>
					<option><xsl:value-of select="columns/column[number($agg.column)]/column_value/text()"/></option>
				</xsl:for-each>
