...
//-=Тело скрипта=-
alert('!!!Процесс назначения курса MBO начат!');
initCodeOrgStructure();
var courses;
courses = XQuery("for $elem in courses where $elem/code='MBO' return $elem");
if (ArrayCount(courses) > 1) {
    alert('В базе найдено больее двух курсов с кодом MBO!. Загрузка остановлена.');
    return;
    ...
    alert('В базе не найден курс с кодом MBO!. Загрузка остановлена.');
    return;
}
var groups, course;
groups = XQuery("for $elem in groups where $elem/code='gMBO1' return $elem");
if (ArrayCount(groups) > 1) {
    alert('В базе найдено больее двух групп с кодом gMBO!. Загрузка остановлена.');
    return;
} else if (ArrayCount(courses) == 1) {
    for (item in groups) {
        idGroup = item.id;
    ...
    alert('В базе не найдено группы с кодом gMBO!. Загрузка остановлена.');
    return;
}
...
linkSourceFile = XQuery("for $elem in resources where $elem/code='listMBO1' return $elem");
