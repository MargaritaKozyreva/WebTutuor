-- Обновление полнотекстового поиска ORACLE #2 для 10g и 11g

-- концепция поменялась из-за старых систем (10g) , теперь так - первый скрипт - вообще не нужен (он для тех кто понимает , а мы таких еще не встречали) , 
-- остальные можно использовать , но они чуть изменились
